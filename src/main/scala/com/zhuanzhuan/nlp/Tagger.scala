...
private[nlp] class Tagger (
  ...
  var nBest = 0  //是否返回NBest结果
  var cost = 0.0  //目前的cost
  var Z = 0.0  //normalize常数
  var obj = 0.0
  var costFactor = 1.0
  ...
  def setCostFactor(costFactor: Double): this.type = {
    this.costFactor = costFactor
    this
  }

  def setNBest(nBest: Int): this.type = {
    this.nBest = nBest
    this
  }

  def read(lines: Sequence, feature_idx: FeatureIndex): this.type = {
    var i: Int = 0
    val tmpLines: Array[Token] = lines.toArray
    while (i < tmpLines.length) {
      val t: Token = tmpLines(i)
      mode match {
        case LearnMode =>
          var j: Int = 0
