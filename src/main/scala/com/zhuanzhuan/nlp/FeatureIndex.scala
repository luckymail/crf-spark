...
private[nlp] class FeatureIndex extends Serializable {
  ...
  def openTagSet(sentence: Sequence): (Array[String], Int) = {
    val tokenNum: Array[Int] = sentence.toArray.map(_.tags.length).distinct
      ...
      tagger.x.foreach { token =>
        if (tagger.x.head != token || templs.head.head.equals('U')) {
          ...
          templs.foreach { templ =>
            val os: String = applyRule(templ, tagger.x.indexOf(token), tagger)
            val id: Int = dic.getOrElse(os, (-1, 0))._1
  ...
  def buildDictionary(tagger: Tagger): mutable.HashMap[String, Int] = {
    val dicLocal: mutable.HashMap[String, Int] = mutable.HashMap[String, Int]()
    List(unigramTempls, bigramTempls).foreach{ templs =>
      tagger.x.foreach{ token =>
        if (tagger.x.head != token || templs.head.head.equals('U'))
          templs.foreach{ templ =>
  ...
  def buildDictionaryDist(taggers: RDD[Tagger],  bcFeatureIdxI: Broadcast[FeatureIndex], freq: Int) {
    ...
    val dictionaryUni: RDD[(String, (Int, Int))] = dictionary.filter(_._1.head == 'U').zipWithIndex()
      .map{ case((feature, frequency), featureID) =>
        (feature, (featureID.toInt * bcFeatureIdxI.value.labels.length, frequency))
      }
    val bcOffSet = taggers.context.broadcast(dictionaryUni.count().toInt * labels.length)
    val dictionaryBi: RDD[(String, (Int, Int))] = dictionary.filter(_._1.head == 'B').zipWithIndex()
      .map{ case((feature, frequency), featureID) =>
        (feature, (featureID.toInt * Math.pow(bcFeatureIdxI.value.labels.length, 2).toInt + bcOffSet.value, frequency))
      }

    val dictionaryGram = dictionaryUni.union(dictionaryBi).collect()

    dictionaryGram.foreach{case(k, v) => dic.update(k, v)}
    maxID = dictionaryGram.map(_._2._1).max + Math.pow(labels.length, 2).toInt
