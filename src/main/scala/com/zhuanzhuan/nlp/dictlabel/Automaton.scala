...
class Automaton extends Serializable {

  // initialization
  val trie: ArrayBuffer[ACNode] = ArrayBuffer(
    ACNode(state = 0,
  ...
  /**
    ...
    */
  private def gotoState(node: ACNode, byte: Byte): Int = {
    ...
    word.getBytes().foreach { t =>
      gotoState(trie(state), t) match {
        case -1 => val next = trie.size
          ...
          trie += ACNode(next, t, mutable.Map[Byte, Int](), 0, mutable.Set[(String, String)]())
          state = next
